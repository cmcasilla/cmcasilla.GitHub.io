...
<div id="content">
							<?php
							 ...
							 while($catres = $catrec->fetch(PDO::FETCH_BOTH)){
							 	...
							 	echo "<tr>";
							 	echo '<td colspan="2">'.'<a href="browse.php?postID='.$catres['postID'].'"  >'.'<img src="'.$catres['Image'].'" width="200" height="150">'.'</a>'.'</td>';
							 	echo "</tr>";
							 	echo "<tr>";
						...
						while($datas = $resultsl->fetch(PDO::FETCH_BOTH)){
							...
							echo '<table cellpadding="-50" cellspacing="-50">';
							echo '<tr>';
							echo '<td rowspan="5"><img src="'.$image.'" width="500" height="350" margin-top:-40px	></td>';
							echo '</tr>';
							echo '<tr>';
							echo '<td>'.$name.'</td>';
							echo '</tr>';
							...
							echo '<td>'.$desc.' '.$cat.'</td>';
							echo '</tr>';
							echo '<div id="comments_section" style="overflow:auto;height:300px;width:500px;z-index:8;">'	;
						$queryc= "SELECT * FROM tbl_comment WHERE postID='".$_GET['postID']."'";
						 ...
						 while($comres = $qrycres->fetch(PDO::FETCH_BOTH)){
							...
							echo '</div>';
  							...
  							<textarea name="comment" style="margin-left:250px;margin-bottom:-39px;position:relative;top:450px;height:100px;width:500px;left:140px;"></textarea>	
  							<input type="submit" Value="Comment" style="margin-top:-39px;position:relative;top:525px;height:50px;width:100px;left:40px;"">
